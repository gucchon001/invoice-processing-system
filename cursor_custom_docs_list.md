# 請求書処理自動化システム - Cursor カスタムドキュメント登録リスト

## Python & フレームワーク

### 1. Streamlit
- **URL**: https://docs.streamlit.io/
- **説明**: メインフレームワーク - UI/UX、サイドバー、ファイルアップロード
- **重要度**: ⭐⭐⭐⭐⭐

### 2. streamlit-aggrid
- **URL**: https://github.com/PablocFonseca/streamlit-aggrid
- **説明**: 高度なデータグリッド表示、インライン編集機能
- **重要度**: ⭐⭐⭐⭐⭐

### 3. Python Typing
- **URL**: https://docs.python.org/3/library/typing.html
- **説明**: 型ヒント（必須要件）
- **重要度**: ⭐⭐⭐⭐

## データベース & ストレージ

### 4. Supabase Python
- **URL**: https://supabase.com/docs/reference/python/introduction
- **説明**: メインデータベースクライアント
- **重要度**: ⭐⭐⭐⭐⭐

### 5. PostgreSQL
- **URL**: https://www.postgresql.org/docs/
- **説明**: Supabaseの基盤データベース
- **重要度**: ⭐⭐⭐⭐

## Google APIs

### 6. Google Drive API Python
- **URL**: https://developers.google.com/drive/api/v3/reference
- **説明**: PDFストレージ操作
- **重要度**: ⭐⭐⭐⭐⭐

### 7. Google Sheets API Python
- **URL**: https://developers.google.com/sheets/api/reference/rest
- **説明**: freee連携シート書き出し
- **重要度**: ⭐⭐⭐⭐

### 8. Google Slides API Python
- **URL**: https://developers.google.com/slides/api/reference/rest
- **説明**: 経理ハンドブック読み取り・ルール抽出
- **重要度**: ⭐⭐⭐⭐⭐

### 9. Google OAuth 2.0 (日本語)
- **URL**: https://developers.google.com/identity/protocols/oauth2?hl=ja
- **説明**: ユーザー認証（日本語ドキュメント）
- **重要度**: ⭐⭐⭐⭐⭐

### 10. Google OAuth 2.0 (English)
- **URL**: https://developers.google.com/identity/protocols/oauth2
- **説明**: ユーザー認証（英語版・技術詳細）
- **重要度**: ⭐⭐⭐⭐

### 11. Google Cloud Authentication
- **URL**: https://cloud.google.com/docs/authentication
- **説明**: サービスアカウント認証
- **重要度**: ⭐⭐⭐⭐

## AI & 自然言語処理

### 12. Gemini API
- **URL**: https://ai.google.dev/docs
- **説明**: AI情報抽出、PDF解析
- **重要度**: ⭐⭐⭐⭐⭐

### 13. Google AI Python SDK
- **URL**: https://ai.google.dev/tutorials/python_quickstart
- **説明**: Gemini API Python実装
- **重要度**: ⭐⭐⭐⭐⭐

## クラウド & デプロイ

### 14. Google Cloud Run
- **URL**: https://cloud.google.com/run/docs
- **説明**: アプリケーションサーバー
- **重要度**: ⭐⭐⭐⭐

### 15. Docker
- **URL**: https://docs.docker.com/
- **説明**: コンテナ化
- **重要度**: ⭐⭐⭐

### 16. Google Artifact Registry
- **URL**: https://cloud.google.com/artifact-registry/docs
- **説明**: コンテナイメージストレージ
- **重要度**: ⭐⭐⭐

## セキュリティ & 設定管理

### 17. Streamlit Secrets
- **URL**: https://docs.streamlit.io/streamlit-community-cloud/get-started/deploy-an-app/connect-to-data-sources/secrets-management
- **説明**: 機密情報管理
- **重要度**: ⭐⭐⭐⭐

### 18. Google Cloud Secret Manager
- **URL**: https://cloud.google.com/secret-manager/docs
- **説明**: 本番環境機密情報管理
- **重要度**: ⭐⭐⭐

## 開発・テスト

### 19. pytest
- **URL**: https://docs.pytest.org/
- **説明**: テストフレームワーク
- **重要度**: ⭐⭐⭐

### 20. Python Logging
- **URL**: https://docs.python.org/3/library/logging.html
- **説明**: ログ出力
- **重要度**: ⭐⭐⭐

## 優先度順登録推奨リスト

### 最優先（⭐⭐⭐⭐⭐）
1. Streamlit
2. streamlit-aggrid  
3. Supabase Python
4. Google Drive API Python
5. Gemini API
6. Google AI Python SDK
7. Google Slides API Python ← 新規追加
8. Google OAuth 2.0 (日本語)

### 高優先（⭐⭐⭐⭐）
9. Python Typing
10. PostgreSQL
11. Google Sheets API Python
12. Google OAuth 2.0 (English)
13. Google Cloud Authentication
14. Google Cloud Run
15. Streamlit Secrets

### 中優先（⭐⭐⭐）
16. Docker
17. Google Artifact Registry
18. Google Cloud Secret Manager
19. pytest
20. Python Logging

## 📋 更新された登録用URL一覧

### 最優先の8つ（今すぐ登録推奨）

```
1. Streamlit
https://docs.streamlit.io/

2. streamlit-aggrid
https://github.com/PablocFonseca/streamlit-aggrid

3. Supabase Python
https://supabase.com/docs/reference/python/introduction

4. Google Drive API Python
https://developers.google.com/drive/api/v3/reference

5. Gemini API
https://ai.google.dev/docs

6. Google AI Python SDK
https://ai.google.dev/tutorials/python_quickstart

7. Google Slides API Python
https://developers.google.com/slides/api/reference/rest

8. Google OAuth 2.0 (日本語)
https://developers.google.com/identity/protocols/oauth2?hl=ja
``` 