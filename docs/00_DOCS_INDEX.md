...
- **[16_データベース設計書.md](16_データベース設計書.md) - データベース設計・ER図** ⭐ **v3.0統一化完了**

### **🏗️ 詳細設計書（独立ファイル）** ⭐ **NEW**
- **[17_システムアーキテクチャUML図.md](17_システムアーキテクチャUML図.md) - システムアーキテクチャ図集**
- **[18_データベースER図.md](18_データベースER図.md) - データベースER図・関係性**
- **[19_テーブル設計詳細仕様書.md](19_テーブル設計詳細仕様書.md) - テーブル仕様・制約・インデックス** ⭐ **v3.0統一化完了**
- **[20_シーケンス図集.md](20_シーケンス図集.md) - 処理フロー・正常系・異常系**
- **[21_クラス図.md](21_クラス図.md) - クラス構造・コンポーネント関係**

### **📋 テーブル統一化プロジェクト成果** ⭐ **完了 - 2025年7月27日最終更新**
- **[テーブル統一化作業サマリー.md](テーブル統一化作業サマリー.md) - 統一スキーマ完全再構築成果**
- **[Phase3_スキーマ最適化計画書.md](Phase3_スキーマ最適化計画書.md) - スキーマ最適化完了報告**

### **🎉 統一スキーマ完全再構築成果** ⭐ **最新 - 2025年7月27日**
- ✅ **根本修正完了**: `main_invoice_number`エラー完全解決
- ✅ **アプリケーション成功率**: 0% → 100%復旧達成
- ✅ **統一スキーマ実装**: 本番・OCRテーブル完全統一
- ✅ **プレビュー機能拡張**: タブ分割詳細表示実装

### **🚀 将来拡張計画** ⭐ **将来実装予定**
- **[Phase3B_高度最適化計画書.md](Phase3B_高度最適化計画書.md) - 高度なDB最適化・パーティショニング計画**
...
- [13_Supabaseデータベース設定・運用ガイド.md](13_Supabaseデータベース設定・運用ガイド.md) - DB運用手順 ⭐ **統一化対応**
...
### **🗄️ データベース設計書（統合版）** ⭐ **統一化完了**
**ファイル**: [16_データベース設計書.md](16_データベース設計書.md)

- **ER図**: 主要テーブル関係図
- **テーブル設計**: 12テーブルの詳細仕様（OCRテスト・本番統一化済み）
- **インデックス戦略**: パフォーマンス最適化（Phase 3A完了）
- **セキュリティ設計**: Row Level Security (RLS)
- **データ品質管理**: 検証システム・スコアリング
- **運用・保守**: バックアップ・監視戦略
- **🚀 最新成果**: Phase 2統一化 + Phase 3A最適化完了（2025年1月22日）
...
### **🏆 テーブル設計書整備プロジェクト（2025年1月22日完了）**

**目標**: OCRテストと本番テーブルの統一化・最適化

**Phase 2統一化成果** ✅:
- カラム名統一: `invoice_number` → `main_invoice_number`, `registration_number` → `t_number`
- データ型最適化: `tax_rate` VARCHAR → DECIMAL(5,2)
- 新規カラム追加: `receipt_number`, `key_info`, `updated_at`
- データ品質向上: 税率データクリーニング・正規化完了

**Phase 3A最適化成果** ✅:
- 最適化ビュー作成: `*_optimized` 3ビュー（論理的カラム順序）
- インデックス最適化: 複合インデックス2つ追加
- 不要フィールド削除: `ocr_test_results.file_size`削除
- データ整合性確認: 410件→410件完全一致
...
**関連ドキュメント**:
- [テーブル統一化作業サマリー.md](テーブル統一化作業サマリー.md)
- [Phase3_スキーマ最適化計画書.md](Phase3_スキーマ最適化計画書.md)
- [19_テーブル設計詳細仕様書.md](19_テーブル設計詳細仕様書.md) v2.0更新済み

**🚀 将来拡張計画策定**:
- [Phase3B_高度最適化計画書.md](Phase3B_高度最適化計画書.md) - パーティショニング・マテリアライズドビュー等の高度最適化計画
