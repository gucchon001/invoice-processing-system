...
**v1.3更新内容**: 40カラム完全設計 + レプリケーション方式対応・新機能カラム反映  
...
**🎉 2025年7月28日更新**: **40カラム完全設計とレプリケーション方式**により、Gmail連携・外貨換算・承認ワークフロー・freee連携の完全基盤が構築されました。マスター・レプリケーション方式により、invoicesテーブル（本番）がマスター、ocr_test_resultsテーブル（テスト）は自動レプリケーションで100%整合性が保証されます。
...
erDiagram
    ...
    INVOICES {
        ...
        varchar source_type "ファイルソース(local/gdrive/gmail)"
        varchar gmail_message_id "GmailメッセージID"
        varchar attachment_id "添付ファイルID"
        varchar sender_email "送信者メール"
        ...
        decimal jpy_amount "円換算金額"
        varchar card_statement_id "カード明細ID"
        ...
        varchar approved_by "承認者"
        timestamp approved_at "承認日時"
        boolean exported_to_freee "freee連携済み"
        timestamp export_date "連携日時"
        varchar freee_batch_id "freeeバッチID"
    }
    ...
    OCR_TEST_RESULTS {
        %% レプリケーション方式: INVOICESテーブル完全レプリケーション + テスト専用カラム
        %% 40カラム（INVOICES完全コピー） + 3カラム（テスト専用） = 43カラム
        uuid id PK "結果ID(UUID)"
        varchar user_email FK "作成ユーザー(レプリケーション)"
        varchar status "処理状況(レプリケーション)"
        timestamp uploaded_at "アップロード日時(レプリケーション)"
        timestamp created_at "作成日時(レプリケーション)"
        timestamp updated_at "更新日時(レプリケーション)"
        varchar file_name "ファイル名(レプリケーション)"
        varchar gdrive_file_id "GoogleDriveファイルID(レプリケーション)"
        varchar file_path "ファイルパス(レプリケーション)"
        varchar source_type "ファイルソース(レプリケーション)"
        varchar gmail_message_id "GmailメッセージID(レプリケーション)"
        varchar attachment_id "添付ファイルID(レプリケーション)"
        varchar sender_email "送信者メール(レプリケーション)"
        varchar issuer_name "発行者名(レプリケーション)"
        varchar recipient_name "請求先名(レプリケーション)"
        varchar main_invoice_number "請求書番号(レプリケーション)"
        varchar receipt_number "受領書番号(レプリケーション)"
        varchar t_number "T番号(レプリケーション)"
        date issue_date "発行日(レプリケーション)"
        date due_date "支払期日(レプリケーション)"
        varchar currency "通貨コード(レプリケーション)"
        decimal total_amount_tax_included "税込合計金額(レプリケーション)"
        decimal total_amount_tax_excluded "税抜合計金額(レプリケーション)"
        decimal exchange_rate "為替レート(レプリケーション)"
        decimal jpy_amount "円換算金額(レプリケーション)"
        varchar card_statement_id "カード明細ID(レプリケーション)"
        jsonb extracted_data "AI抽出データJSON(レプリケーション)"
        jsonb raw_response "生のAI応答JSON(レプリケーション)"
        jsonb key_info "キー情報JSON(レプリケーション)"
        boolean is_valid "検証状況(レプリケーション)"
        text[] validation_errors "検証エラー配列(レプリケーション)"
        text[] validation_warnings "検証警告配列(レプリケーション)"
        decimal completeness_score "完全性スコア(レプリケーション)"
        decimal processing_time "処理時間(レプリケーション)"
        varchar approval_status "承認状況(レプリケーション)"
        varchar approved_by "承認者(レプリケーション)"
        timestamp approved_at "承認日時(レプリケーション)"
        boolean exported_to_freee "freee連携済み(レプリケーション)"
        timestamp export_date "連携日時(レプリケーション)"
        varchar freee_batch_id "freeeバッチID(レプリケーション)"
        %% テスト専用カラム（3個）
        uuid session_id FK "セッションID(テスト専用)"
        varchar gemini_model "Geminiモデル(テスト専用)"
        varchar test_batch_name "テストバッチ名(テスト専用)"
    }
    ...
    OCR_TEST_LINE_ITEMS {
        ...
        text item_description "商品・サービス名(統一)"
        decimal quantity "数量"
        decimal unit_price "単価"
        decimal amount "金額"
        decimal tax_rate "税率(統一:DECIMAL)"
        timestamp created_at "作成日時"
        timestamp updated_at "更新日時(統一)"
    }
...
**承認者**: データベース設計者  
**レビュー予定**: 2025年8月27日  
**v1.2変更**: 統一スキーマ完全再構築によるフィールド名統一対応
