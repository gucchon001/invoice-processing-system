...
**v1.2更新内容**: 統一スキーマ完全再構築対応・フィールド名統一反映  
...
**🎉 2025年7月27日更新**: **統一スキーマ完全再構築**により、本番・OCRテストテーブル間の完全統一が達成されました。主要フィールド名（`main_invoice_number`, `t_number`, `item_description`等）とデータ型（`tax_rate`のDECIMAL統一）が完全に整合されています。
...
erDiagram
    ...
    OCR_TEST_RESULTS {
        ...
        varchar recipient_name "請求先名"
        varchar receipt_number "受領書番号"
        varchar main_invoice_number "請求書番号(統一)"
        varchar t_number "T番号(統一)"
        varchar currency "通貨"
        decimal total_amount_tax_included "税込金額"
        decimal total_amount_tax_excluded "税抜金額"
        date issue_date "発行日"
        date due_date "支払期日"
        jsonb key_info "キー情報(統一)"
        boolean is_valid "検証状況"
        decimal completeness_score "完全性スコア"
